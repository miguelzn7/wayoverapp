...
const ImportEditor = ({ params, onNavigate }) => {
  ...
  return (
    <div style={{ paddingBottom: '160px', maxWidth: '900px', margin: '0 auto' }}>

      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '16px 40px', borderBottom: '1px solid #fbfdf9', position: 'sticky', top: 0, background: 'white', zIndex: 100 }}>
        <button onClick={() => onNavigate('insta-import')} style={{ background: 'none', border: 'none', cursor: 'pointer' }}><ArrowLeft size={24} /></button>
        <div style={{ background: '#fbfdf9', padding: '6px 16px', borderRadius: '20px', fontSize: '0.9rem', fontWeight: '600' }}>Item {currentIndex + 1} of {queue.length}</div>
        <div style={{ width: 24 }}></div>
      </div>

      {totalUploads > 0 && (
        <div style={{ background: uploadingCount > 0 ? '#fbfdf9' : '#fbfdf9', padding: '12px 20px', display: 'flex', gap: '12px', fontSize: '0.9rem' }}>
          {uploadingCount > 0 ? <Upload size={20} className="spin" /> : <CheckCircle size={20} color="#155e31" />}
      ...
      <div style={{ position: 'relative', width: '100%', background: '#fbfdf9', marginBottom: '24px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              <div key={idx} style={{ position: 'relative' }}>
                <OptimizedImage
                  ...
                  style={{
                    ...
                    border: idx === imageIndex ? '2px solid #155e31' : '1px solid #ddd',
      ...
      <div style={{ padding: '0 20px' }}>
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
            ...
            />
            <button onClick={addTag} disabled={(currentItem.tags || []).length >= 5 || !currentTag.trim()} style={{ padding: '8px 14px', background: '#155e31', color: '#fbfdf9', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>Add</button>
          </div>
      ...
      <div style={{ position: 'fixed', bottom: '170px', left: 0, right: 0, display: 'flex', justifyContent: 'center', zIndex: 100, padding: '0 20px', pointerEvents: 'none' }}>
        <button
          onClick={handleNext}
          style={{
            background: '#155e31', color: '#fbfdf9', border: 'none', padding: '16px 32px', borderRadius: '30px',
            fontSize: '1rem', fontWeight: 'bold', boxShadow: '0 4px 12px rgba(21, 94, 49, 0.4)',
