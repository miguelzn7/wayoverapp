...
const InstagramImport = ({ onNavigate }) => {
  ...
  return (
    <div style={{ padding: '20px', paddingBottom: '160px', maxWidth: '900px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '32px', alignItems: 'center' }}>
        ...
        <button
          onClick={handleFetch}
          disabled={loading}
          style={{ padding: '12px 24px', background: '#155e31', color: '#fbfdf9', border: 'none', borderRadius: '8px', cursor: 'pointer' }}
        >
          ...
          return (
            <div
              ...
              style={{
                ...
                border: isSelected ? '3px solid #155e31' : '1px solid #e5e7eb',
            ...
            >
              ...
              {isSelected && (
                <div style={{ position: 'absolute', inset: 0, background: 'rgba(16, 185, 129, 0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <CheckCircle size={48} color="#fbfdf9" fill="#155e31" />
                </div>
              )}
            </div>
          );
      ...
      {selectedIds.length > 0 && (
        <div style={{ position: 'fixed', bottom: '230px', left: 0, right: 0, display: 'flex', justifyContent: 'center', zIndex: 100, padding: '0 20px', pointerEvents: 'none' }}>
          <button
            ...
            disabled={loading}
            style={{
              background: '#155e31', color: '#fbfdf9', border: 'none', padding: '16px 32px', borderRadius: '30px', fontSize: '1rem', fontWeight: 'bold', boxShadow: '0 4px 12px rgba(21, 94, 49, 0.4)', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1, pointerEvents: 'auto'
            }}
          >
